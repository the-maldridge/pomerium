// Package evaluator defines a OriginalEvaluator interfaces that can be implemented by
...
// OriginalEvaluator specifies the interface for a policy engine.
type OriginalEvaluator struct {
...
// NewOriginalEvaluator creates a new OriginalEvaluator.
func NewOriginalEvaluator(options *config.Options, store *Store) (*OriginalEvaluator, error) {
	e := &OriginalEvaluator{
